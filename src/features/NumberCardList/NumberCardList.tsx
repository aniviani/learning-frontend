...
const handleAddedNumberChange: ChangeEventHandler<HTMLInputElement> = (event) => {
...
console.log(addedNumber)
    ...
    return (
        <>
        <div>
            <input type="number" placeholder="введите число" value={addedNumber} onChange={handleAddedNumberChange}/>
            <button onClick={handleNumberAdd}>добавить</button>
        </div>
        
        <div>
            {numbers.map((number) => (
                <NumberCard key={number} number={number} onDelete={handleNumberDelete}/>
            ))}
        </div>
        </>
    )
